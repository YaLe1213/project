 ...
 <body>
  <div class="mws-panel grid_8 mws-collapsible"> 
   <div class="mws-panel-header"> 
    <span><i class="icon-table"></i> 快递方式列表</span> 
      ...
      <table class="mws-table mws-datatable dataTable" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info"> 
       <thead> 
        <tr role="row">
         ...
         <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 207px;">快递名</th>
        ...
        <tr class="odd"> 
         ...
         <td class=" ">
           <select class="select" name="{{$row->id}}">
             <option value="0" @if($row->Flag==0) selected @endif>未启用</option>
             <option value="1" @if($row->Flag==1) selected @endif>启用</option>
           </select>
         </td>  
         <td class=" "> 
          <span class="btn-group">  
            
            <a href="/send/{{$row->id}}/edit" class="btn btn-success">快递修改</a>
            <a href="/send/{{$row->id}}" class="btn btn-success">删除</a>
...
<script>
  ...
  $(".select").change(function(){
    ...
    // alert(id);
    // ajax
    $.get("/sendstatus",{id:id,sta:sta},function(data){
      if (data==1) {
        alert("修改成功");
      }else if(data==2){
        alert("修改失败");
